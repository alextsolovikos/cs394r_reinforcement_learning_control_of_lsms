...
class MultipleParticlesInFlowContinuous(gym.Env):
...
#       self.obs_1d = True if np.prod(self.n_cells) == 1 else False
        self.obs_model = ObservationModel2D(min_coord=self.obs_min.tolist(), max_coord=self.obs_max.tolist(), n_cells=self.n_cells)
        self.observation_space = gym.spaces.Box(low=np.zeros(np.prod(self.n_cells)), high=np.ones(np.prod(self.n_cells)), dtype=np.float64)
#       if self.obs_1d:
#           self.observation_space = gym.spaces.Box(low=np.zeros([1]), high=np.ones([1]), dtype=np.float64)
#       else:
#           self.observation_space = gym.spaces.Box(low=np.zeros(self.n_cells + [1]), high=np.ones(self.n_cells + [1]), dtype=np.float64)
...
#           return self.obs_model(state).flatten().astype(np.float64) / self.n_particles
#       else:
#           return self.obs_model(state).astype(np.float64).reshape(*self.n_cells,1) / self.n_particles
...
#         return np.exp(-(x - self.xc)**2/2./self.sigma_x**2) * np.exp(-(y - self.yc)**2/2./self.sigma_y**2)
    ...
    def step(self, action):
        ...
        """
        self.episode_len += 1
        self.cur_action = self.cur_action + self.lamb * (action[0] - self.cur_action)
...
#         reward = -self.cur_state[1] if done else - self.act_cost_weight * action[0]
        ...
        self.cur_reward_actuation = -action[0]
...
#         reward = -(uy-self.v_target)**2 - self.act_cost_weight * action[0]**2
        ...
        return self.cur_obs, self.cur_reward_total, done, {}
